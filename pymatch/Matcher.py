...
class Matcher:
    ...
    def __init__(self, test, control, yvar, formula=None, exclude=[]):
        ...
        self.minority, self.majority = [
            i[1] for i in sorted(zip([self.testn, self.controln], [1, 0]), key=lambda x: x[0])
        ]
    ...
    def fit_scores(self, balance=True, nmodels=None):
        ...
        if balance:
            if nmodels is None:
                ...
                minor, major = [
                    self.data[self.data[self.yvar] == i] for i in (self.minority, self.majority)
                ]
            ...
            while i < nmodels and errors < 5:
                ...
                df = pd.concat(
                    [
                        uf.drop_static_cols(df[df[self.yvar] == 1], yvar=self.yvar),
    ...
    def prop_test(self, col):
        ...
        if not uf.is_continuous(col, self.X) and col not in self.exclude:
            ...
            pval_after = round(
                stats.chi2_contingency(self.prep_prop_test(self.matched_data, col))[1], 6
            )
    ...
    def compare_continuous(self, save=False, return_table=False):
        ...
        for col in self.matched_data.columns:
            if uf.is_continuous(col, self.X) and col not in self.exclude:
                ...
                ax1.set_title(
                    title_str.format(
                        col, "before", ksb, pb, std_diff_med_before, std_diff_mean_before
                    )
                )
                ax2.plot(xca.x, xca.y, label="Control")
                ax2.plot(xta.x, xta.y, label="Test")
                ax2.set_title(
                    title_str.format(col, "after", ksa, pa, std_diff_med_after, std_diff_mean_after)
                )
